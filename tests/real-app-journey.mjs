...
try{
 ...
 assert.equal(new URL(page.url()).origin,appOrigin);assert.equal(await page.locator('#connection').textContent(),'connected');assert.match(await page.locator('#world-state').textContent(),/runtime mismatch/i);assert.match(await page.locator('#portfolio-summary').textContent(),/3 active/i);assert.match(await page.locator('#portfolio-summary').textContent(),/1 incubating/i);await page.screenshot({path:out+'/real-app-world-phone.png'});
