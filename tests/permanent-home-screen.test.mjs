...
assert.equal(runtime.scope, 'private-tailnet');
assert.equal('origin' in runtime, false, 'public discovery must not disclose the local bind target');
...
assert.match(appHtml, /const CANONICAL_APP_PATH="\/q-launcher\/app\/"/);
assert.match(appHtml, /u\.pathname=CANONICAL_APP_PATH/);
assert.match(appHtml, /const UPDATE_RETRY_MS=60000/);
assert.match(appHtml, /scheduleUpdateRetry/);
assert.match(appHtml, /app-retry/);
assert.match(appHtml, /addEventListener\("online",\(\)=>\{checkForUpdate\(\);boot\(false\)\}\)/);
